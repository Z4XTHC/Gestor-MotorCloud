...
import { obtenerDatosOrden } from "../../api/ordenApi";
...
export const Hero = () => {
  ...
  const handleConsulta = async (e: React.FormEvent) => {
    ...
    if (ordenNumero.trim()) {
      setLoading(true);
      try {
        const data = await obtenerDatosOrden(ordenNumero.trim());
        if (data && data.numeroOrden) {
          setOrden(data);
          setModalOpen(true);
        } else {
          showError(
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder="Ingresá tu número de OT-XXXXXXXX"
        ...
        <div className="flex flex-wrap justify-center gap-4">
          ...
          <a
            href={`https://wa.me/+549${empresa?.telefono || ""}`}
            target="_blank"
      ...
      {modalOpen && orden && orden.numeroOrden && (
        <OrderDetalles orden={orden} onClose={() => setModalOpen(false)} />
      )}
    </section>
  );
};
