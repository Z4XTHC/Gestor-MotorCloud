 ...
 * @param payload - Objeto con los nuevos datos de la empresa.
 * @returns Una promesa que se resuelve con los datos actualizados de la Empresa.
 */
export async function actualizarDatosEmpresa(
  payload: Partial<Empresa>,
): Promise<Empresa> {
  const { data } = await axiosInstance.put(
    API_ENDPOINTS.EMPRESA.UPDATE,
    payload,
  );
  return data;
}
